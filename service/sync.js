    ...
    return {
        items: function(req, res, next) {
            new sql.Request(conn)
                .query(`SELECT SK_NO,SK_NAME,SK_BCODE FROM SSTOCK WHERE 
						LEN(SK_BCODE) > 12 
        ...
        // SELECT OD_CTNO,FirstSale,Price,Amount,SK_NO,SK_BCODE,SK_NAME,SK_SPEC,SK_UNIT,SK_SUPPNO,SK_SUPPNAME,SK_LOCATE,SK_LPRICE1,SK_LPRICE2,SK_IKIND,SK_NOWQTY,SK_KINDNAME 
        // 				FROM (         
        // 				   SELECT  
        ...
        // 					,a.*
        // 					,OD_CTNO
        // 					,rank() OVER (PARTITION BY SK_NO,OD_CTNO ORDER BY sd.OD_DATE1 DESC) r
        // 					FROM 
        ...
        // 					) t WHERE r = 1 AND OD_CTNO IS NOT NULL ORDER BY OD_CTNO

        customerOrderHistory: function(req, res, next) {
            new sql.Request(conn)
                .query(`
						DECLARE @sstock table(  
						    SK_NO nvarchar(30),  
